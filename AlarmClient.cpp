...
int AlarmClient::start()
{
    ...
    while(1)
    {
        ...
        /* If user insert his persanal RFID card */
        if(this->card_reader.data_available())
        {
            ...
            if(this->usermap.find(tmp_user) != usermap.end())
            {
                ...
                // PowerOff if poweroff card inserted in reader
                if(tmp_user == "PowerOff ")
                {
                    this->led.Error(OFF);
                    send_power_off();
                    this->led.Ok(ON);
                    ...
                    std::cout << "cant shutdown system" << std::endl;
                }
                else if(tmp_user != this->current_user)
                {
                                        // Green led On
        ...
        /* if power up */
        if(power_on.isPressed() && (this->is_on == 0))
        {
            this->is_on = 1;
            this->led.Error(ON);
            while(this->send_power_on() < 0)
            {
...
int AlarmClient::send_alarm()
{
    ...
    if(err < 0)
        return -1;
    /* send info message (XML file name) */
...
int AlarmClient::send_power_on()
{
    // prepare data to send
    this->xml.result = "Power On";
    this->pack_data();
...
int AlarmClient::send_power_off()
{
    // prepare data to send
    this->xml.result = "Power Off";
    this->pack_data();
